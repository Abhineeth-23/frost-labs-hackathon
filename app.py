...
# ==========================================
#          GOOGLE GEN AI FEATURES
# ==========================================

@app.route('/step/<int:step_id>/generate_ai_tasks', methods=['POST'])
@login_required
def generate_ai_tasks(step_id):
    step = Step.query.get_or_404(step_id)
    if step.user_id != current_user.id:
        return jsonify({'error': 'Unauthorized'}), 403

    if not os.environ.get('GOOGLE_API_KEY'):
        flash("Google API Key missing in settings.", "error")
        return redirect(url_for('step_view', step_id=step.id))
    ...
    try:
        ...
        prompt = (f"I have a goal: '{step.title}'. "
                  f"Break this down into 5 short, actionable subtasks. "
                  f"Return ONLY a valid JSON array of strings (e.g. ['Task A', 'Task B']). "
        ...
        for task_text in tasks:
            ...
            exists = SubTask.query.filter_by(step_id=step.id, text=task_text).first()
            if not exists:
                db.session.add(SubTask(text=task_text, step=step))
                count += 1
        
        db.session.commit()
        flash(f"✨ AI Architect created {count} new tasks!", "success")

    except Exception as e:
        print(f"AI Error: {e}")
        flash("The AI is having trouble thinking right now. Try again.", "error")

    return redirect(url_for('step_view', step_id=step.id))

@app.route('/step/<int:step_id>/diagnose', methods=['POST'])
@login_required
def diagnose_step(step_id):
    step = Step.query.get_or_404(step_id)
    if step.user_id != current_user.id:
        return jsonify({'error': 'Unauthorized'}), 403

    if not os.environ.get('GOOGLE_API_KEY'):
        flash("Google API Key missing.", "error")
        return redirect(url_for('step_view', step_id=step.id))

    # Gather Context
    logs = StepLog.query.filter_by(step_id=step.id).order_by(StepLog.date.desc()).limit(7).all()
    log_txt = "\n".join([f"- {l.date}: {l.content}" for l in logs])
    
    # Simple Prompt
    prompt = (f"Analyze this goal: '{step.title}'.\n"
              f"Recent Activity:\n{log_txt}\n\n"
              f"Give 1 paragraph of 'Tough Love' advice on how to improve consistency. "
    ...
    try:
        ...
        flash(advice_html, "diagnostic") # Special category 'diagnostic'
    except Exception as e:
        flash("Analysis failed. Try again later.", "error")
